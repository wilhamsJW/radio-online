...
// Função getServerSideProps para pré-carregar dados
export const getServerSideProps: GetServerSideProps = async (context) => {
  const queryClient = new QueryClient();
  const currentPage = parseInt(context.query.page as string, 10) || 1;

  // Usar fetchStationsApi para obter dados
  await queryClient.prefetchQuery(['stations', currentPage], () => fetchStationsApi(currentPage));
...
const ListRadioStations: React.FC = () => {
  ...
  const handleRadioClick = (newStationFavorite: RadioStation) => {
    ...
    const { changeuuid } = newStationFavorite;
    if (changeuuid) {
      setCheckedMusic((prevChecked) => {
        if (prevChecked.includes(changeuuid)) {
          // Remove da lista se já estiver selecionada
          return prevChecked.filter(id => id !== changeuuid);
        } else {
          // Adiciona à lista se não estiver selecionada
          return [...prevChecked, changeuuid];
        }
      });
    }
      ...
      <VStack spacing={3} align="start">
        {paginatedRadios.length > 0 ? (
          paginatedRadios.map((station: RadioStation, index: number) => (
            ...
            >
              {checkedMusic.includes(station?.changeuuid ? station?.changeuuid : '') && <CheckedPlaylist />}
